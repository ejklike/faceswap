...
class Trainer():
    ...
    def train_one_step(self, epoch, save_image):
        # ---------------------
        ...
        print('[%s][%d] Loss_DA: %f Loss_DB: %f Loss_GA: %f Loss_GB: %f'
              % (time.strftime("%H:%M:%S"), epoch, self.errDA_sum/self.avg_counter, self.errDB_sum/self.avg_counter, self.errGA_sum/self.avg_counter, self.errGB_sum/self.avg_counter),
              end='\r')

        if save_image is True:
            self.show_sample(target_A, target_B, epoch)
    ...
    def show_sample(self, tA, tB, epoch, n_show=10):
        ...
        def showG(test_A, test_B, path_A, path_B):
            ...
            figure = figure.reshape((4, n_show // 2) + figure.shape[1:])
        ...
        def display_fn(figure, name, epoch):
            cv2.imwrite(str(self.model.model_dir / '{}_{}.png'.format(name, epoch)), figure)

        tA = tA[0:n_show]
        tB = tB[0:n_show]

        display_fn(showG(tA, tB, self.path_A, self.path_B), "masked", epoch)
        display_fn(showG(tA, tB, self.path_bgr_A, self.path_bgr_B), "raw", epoch)
        display_fn(showG_mask(tA, tB, self.path_mask_A, self.path_mask_B), "mask", epoch)
        # Reset the averages
