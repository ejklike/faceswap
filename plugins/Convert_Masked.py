...
class Convert():
    ...
    def hist_match(self, source, template, mask=None):
        ...
        s_values, bin_idx, s_counts = numpy.unique(source, return_inverse=True, return_counts=True)
        t_values, t_counts = numpy.unique(template, return_counts=True)
        ms_values, mbin_idx, ms_counts = numpy.unique(source, return_inverse=True, return_counts=True)
    ...
    def get_image_mask(self, image, new_face, face_detected, mat, image_size):
        ...
        if 'hull' in self.mask_type:
            hull = cv2.convexHull( numpy.array( face_detected.landmarksXY ).reshape((-1,2)).astype(int) ).flatten().reshape( (-1,2) )
